...
export async function POST(request: Request) {
  ...
  const { data: max } = await supabase.from("lead_stages").select("order_index").order("order_index", { ascending: false }).limit(1).single();
  const order_index = (max?.order_index ?? -1) + 1;
  const { data, error } = await supabase.from("lead_stages").insert({
    name,
    slug,
    order_index,
    is_meeting_stage: Boolean(body?.is_meeting_stage),
    requires_meeting: Boolean(body?.requires_meeting),
    is_won: Boolean(body?.is_won),
    is_lost: Boolean(body?.is_lost),
    color: (body?.color as string)?.trim() || null,
  }).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json(data);
}
...
export async function PATCH(request: Request) {
  ...
  for (let i = 0; i < order.length; i++) {
    await supabase.from("lead_stages").update({ order_index: i }).eq("id", order[i]);
  }
